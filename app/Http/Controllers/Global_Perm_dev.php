...
class Global_Perm_dev extends Controller
{
	//
...
public function checkUserProp($sSession){
	...
	$sSQL = "select a.userid,b.session,c.id_group,c.main
...
public function checkUserPropMain($sSession){ 
	...
	$sSQL = "
			select a.userid,b.session,c.id_group,c.main,d.position_id,d.emp_number,$clumn,d.emp_id,d.id as his_id
			from 
...
public function checkMenuProp($sysInfo){  
	...
	//$groupname = $v['groupname'];
		...
		$sSQL2 = "
			 select h.name,h.id_group,x.prop
			 from 
			 (
			 select z.id,y.name,z.id_group,y.model_menu
			 ...
			 group by x.prop,h.name,h.id_group
			";
		...
		foreach($aryRes as $k=>$v){
			...
			foreach($v as $x=>$y){
				switch($x){
					...
					case "posting":
					if($y == "true"){
						$rate_period = $sys_prop[0]['ex_rate_period'];
						if($rate_period == "DAILY"){
							$date 		= date("Y-m-d");
							$sSLDate 	= "and a.date = '$date'";
						}else if($rate_period == "MONTHLY"){ 
							$date 		= date("Y-m");
							$sSLDate 	= "and a.date like '$date%'";
						}else{ 
							$date 		= date("Y-m-d");
							$sSLDate 	= "and a.date = '$date'";
						}
						$sSQL = "select a.id
								from $trs_curr_conv a
								where 1=1 
								$sSLDate
								";
						$query = $obj->db->query($sSQL);
