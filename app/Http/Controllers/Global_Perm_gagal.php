...
class Global_Perm extends Controller
{
	...
	public function createMenu($sSession){
		...
		$sSQL2 = "
						select d.id,d.description,d.name,d.icon,d.model,d.model_menu,d.model_id,d.parent
						from 
						(
						select a.userid  
						from $table_session b,$table_user a
		...
		*/
		$array3 	= json_decode(json_encode($results2), true);
			...
			foreach($array2 as $key2 => $val2){
				...
				if($id_par == $parent){
					...
					foreach($array3 as $key3 => $val3){
						$id_sib  		= $val2['id'];
						$parent_sib 	= $val3['parent'];
						
						if($id_sib == $parent_sib){
